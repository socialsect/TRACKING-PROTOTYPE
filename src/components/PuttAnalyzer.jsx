...
const MAX_PUTTS = 3; // Changed from 5 to 3
...
class GolfBallKalmanFilter {
  ...
  filter(measurement) {
    ...
    if (!measurement || measurement.length < 2) {
        if (this.initialized && this.confidence > 0.2) { // Lowered from 0.3
...
export default function PuttAnalyzer() {
    ...
    const getStartingPoint = () => ({
        x: getCenterLineX(),
        y: canvasDimensions.height * 0.8 // 80% down from top
    });
    ...
    // TEMPORARY: Accept ALL detections to see what's being found
  const isValidBallDetection = (detection, videoRect, previousDetections = []) => {
    ...
    // LOWERED threshold and removed movement requirements for stationary ball detection
    const isValid = detection.confidence > 0.25; // Even lower threshold
    ...
    // Detection processing with enhanced filtering
    useEffect(() => {
                ...
                tempCanvas.toBlob(async (blob) => {
                    ...
                    try {
                        ...
                        if (detections?.length > 0) {
                            ...
                            // Filter valid detections with VERY loose criteria
                            ...
                            if (validDetections.length > 0) {
                                ...
                                // Scale coordinates
                                const scaleX = videoRect.width / (videoRef.current.videoWidth * scale);
                                const scaleY = videoRect.height / (videoRef.current.videoHeight * scale);
                                ...
                                if (bestDet.box && bestDet.box.length === 4) {
                                    ...
                                    console.log(`📦 Bounding box: [${lastDetectionBox.current.map(v => v.toFixed(1)).join(', ')}]`);
                                }
                            } else {
        ...
        const handleMissedDetection = () => {
            ...
            // Use prediction for short gaps with golf ball physics
            ...
            // Clear bounding box after missed detections
            if (framesSinceDetection.current > 5) {
                lastDetectionBox.current = null;
            }
        };

        // Balanced interval for good performance
    ...
    useEffect(() => {
                ...
                for (let i = 0; i < smoothedPath.length; i++) {
                    ...
                    if (point.x !== undefined && point.y !== undefined) {
                        if (!started) {
                            ...
                            // Different style for predictions
                            if (point.predicted) {
                                ctx.setLineDash([8, 8]);
        ...
        // Draw detection bounding box (only for valid balls)
        if (lastDetectionBox.current && framesSinceDetection.current < 3) {
            ...
            ctx.shadowColor = "black";
            ctx.shadowBlur = 1;
            ctx.fillText("Golf Ball", x1, y1 - 3);
            ctx.shadowBlur = 0;
        }
    ...
    useEffect(() => {
        if (puttCount >= MAX_PUTTS && completedPaths.length > 0) {
            ...
            // Calculate direction and dispersion for each putt
            const puttAnalyses = completedPaths.map(path => {
                ...
                // Calculate direction angle from start to end relative to center line
                const deltaX = endPoint.x - startPoint.x;
                const deltaY = endPoint.y - startPoint.y;
                const angleRadians = Math.atan2(deltaX, -deltaY); // -deltaY because Y increases downward
                const angleDegrees = angleRadians * (180 / Math.PI);

                // Calculate dispersion (distance from end point to center line)
